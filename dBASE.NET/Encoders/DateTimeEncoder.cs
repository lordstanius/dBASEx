...
{
    internal class DateTimeEncoder : Encoder
    {
        public DateTimeEncoder(Encoding encoding) : base(encoding) { }
        ...
        private static DateTime ConvertFoxProToDateTime(ArraySegment<byte> bytes)
        {
            ...
            uint dateWord = BitConverter.ToUInt32(bytes.Array, bytes.Offset);
            int timeWord = (int)BitConverter.ToUInt32(bytes.Array, bytes.Offset + 4);
        ...
        {
            return DateTime.FromBinary(long.Parse(value, CultureInfo.InvariantCulture));
        }

        public override string ToString(object value)
        {
            return base.ToString(((DateTime)value).ToBinary());
        }
    }
}
